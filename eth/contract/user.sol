   ...
   require(!isExitUserEmailFunc(_email), "Email already exists! please use another email");
    ...
    if(_addr == administrator){
            ...
            userStructMap[_addr] =  UserStruct({
                ...
                usertype: UserType.administrator,    // 超级管理员
                exist: true
            });
...
function isAdminFunc(address _addr) public view returns (bool) {
    ...
    return userStructMap[_addr].usertype <= UserType.admin; // 判断用户是否为管理员
}
...
function addAdminFunc(address _addr) public onlyAdministrator {
    ...
    userStructMap[_addr].usertype = UserType.admin;
...
function removeAdminFunc(address _addr) public onlyAdministrator {
    ...
    userStructMap[_addr].usertype = UserType.normal; // 将管理员降级为普通用户
    // 将管理员移除   
...
// 用户查看自己的信息  
function getMyselfInfoFunc() public view returns(address,string memory,string memory,uint256,UserType) {
